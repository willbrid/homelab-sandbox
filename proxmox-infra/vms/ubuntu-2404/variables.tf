...
variable "template_vm_id" {
  description = "ID du template Ubuntu 24.04 à cloner."
  type        = number
  default     = 9000
}
...
variable "cloud_init_package_update" {
  description = "Met à jour la liste des paquets au premier boot (apt update). Recommandé : false."
  type        = bool
  default     = false
}

variable "cloud_init_package_upgrade" {
  description = "Met à jour les paquets installés au premier boot (apt upgrade). Recommandé : false."
  type        = bool
  default     = false
}
...
variable "vms" {
  description = <<-EOT
    Map des VMs Ubuntu 24.04 à créer. La clé est le nom de la VM.
