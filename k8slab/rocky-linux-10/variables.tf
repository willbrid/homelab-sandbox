...
variable "template_vm_id" {
  description = "ID du template Rocky Linux 10 à cloner."
  type        = number
  default     = 9002
}
...
variable "cloud_init_package_upgrade" {
  description = <<-EOT
    Met à jour les paquets installés au premier boot (dnf upgrade). Recommandé : false.
    Sur Rocky Linux 10, laisser à true peut provoquer des timeouts du provider
    (dnf upgrade + SELinux relabeling peuvent dépasser 30 min au premier boot).
  EOT
  type    = bool
  default = false
}
...
variable "timeout_start_vm" {
  description = <<-EOT
    Timeout du démarrage VM en secondes. Rocky Linux 10 nécessite plus de temps
    au premier boot (cloud-init + SELinux relabeling) avant que le guest agent
    ne réponde. Valeur recommandée : 3600.
  EOT
  type    = number
  default = 3600
}
...
variable "vms" {
  description = <<-EOT
    Map des VMs Rocky Linux 10 à créer. La clé est le nom de la VM.
  ...
  type = map(object({
    ...
    # Disques supplémentaires (scsi1 …), vierges : ni partitionnés, ni formatés,
    # ni montés — typiquement le disque brut dédié au DiskPool OpenEBS/Mayastor
    # des workers. Contraintes et valeurs par défaut : var.extra_disks du module
    # proxmox-vm, qui porte les validations.
